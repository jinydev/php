...
> [!NOTE]
> PHP 7.x 및 8.x부터는 스칼라 타입 힌팅 외에 엄격한 타입 검사(`strict_types`), 여러 타입을 동시에 허용하는 유니온 타입(`Union Types`) 등을 활용할 수 있는 더욱 고도화된 정적 타입 시스템을 지원합니다. 자세한 내용은 **[매개변수 및 반환값 타입 선언](types)** 페이지에서 깊이 있게 다룹니다.
...
<div style="text-align: center; margin: 30px 0;">
  <img src="img/call_by_reference_cartoon.png" alt="Call by Value vs Call by Reference Cartoon" style="max-width: 80%; height: auto; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);" />
  <p style="font-size: 13px; color: #64748b; margin-top: 8px;">그림: 값 복사 전달(Call by Value, 복사본 전달)과 참조 주소 전달(Call by Reference, 원본 전달)의 대조</p>
</div>
...
> [!TIP]
> **PHP 8.0 이상을 사용하고 계신가요?**  
> 인자 값의 개수가 많거나 기본값(Default)을 가진 인자들을 효율적으로 지정해서 넘겨주고 싶다면, 매개변수 이름을 명시하여 호출하는 **[네임드 인자 (Named Arguments)](named)** 단원을 학습해 보세요.
