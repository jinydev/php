...
![switch 흐름도](./img/switch_flow.svg)
...
> 기존 `switch` 문법의 엄격하지 않은 타입 비교(`==`) 문제와 복잡한 `break;` 생략 실수를 보완한 현대적이고 안전한 **[Match 표현식](match)**이 새롭게 도입되었습니다. 분기별 반환값 연산이 필요하다면 `match` 표현식을 함께 학습해 보시는 것을 적극 권장합니다.
